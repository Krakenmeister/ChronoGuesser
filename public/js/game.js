...
const headlines = [
  ...
  ["Wonder Kid Helps Americans Around the Clock", "The Meteoric Rise of Chrono Delivery Kid"],
];
...
async function settings() {
  ...
  document.getElementById("effectsVolume").addEventListener("change", (event) => {
    ...
    trainSound.volume = effectsVolume / 100;
...
async function startGame(numPapers, timeLimit) {
  let endGame = async () => {
    ...
    let paperPromises = [];
    for (let i = 0; i < numPapers; i++) {
      paperPromises.push(axios.get(`http://www.loc.gov/item/${documents[i].locId}/${documents[i].date.year}-${documents[i].date.month}-${documents[i].date.day}/ed-1/?fo=json&st=pdf`));
    }
    ...
    await Promise.all(paperPromises).then((info) => {
      for (let i = 0; i < numPapers; i++) {
        if (!info[i].data.item.location_state || !info[i].data.item.location_city) {
          newspapersInfo.push({
    ...
    document.getElementById("homeWrapper").innerHTML = `
        <div id="scoreWrapper">
          <div id="scoreTitle">
            ...
            <p id="scoreSubtitle">The Time Traveler's Digest</p>
          </div>
    ...
    let articleTexts = [
      ...
      [
        ...
        certainly recommend the Chrono Delivery Kid to their friends and neighbors!`,
        `The Time Council congratulates Chrono Delivery Kid for delivering NEWSPAPER_TITLE to GUESS_MONTH GUESS_DAY, GUESS_YEAR despite some unfortunate ink stains obscuring the date
        ...
        `Chrono Delivery Kid saves the day for the FAMILY_PLURAL! No one local to their time could deliver an issue of NEWSPAPER_TITLE, so our hero delivered it to them on
        the GUESS_DAY of GUESS_MONTH, GUESS_YEAR. The copy they received was from DOCUMENT_MONTH DOCUMENT_DAY, DOCUMENT_YEAR, so they were REACTION_GOOD. News spreads of this wonder kid
        as their delivery services are requested more and more.`,
      ],
    ];

    let imageCaptions = [
      `Photograph of Chrono Delivery Kid in action, circa ${Math.floor(Math.random() * 170) + 1790}. Received ${score} out of ${1000 * numPapers} points for their deliveries.`,
      `Chrono Delivery Kid after a long day of deliveries. Received ${score} out of ${1000 * numPapers} points.`,
      `Chrono Delivery Kid awarded ${score} out of ${1000 * numPapers} points, receiving special congratulations by the Time Council.`,
    ];
    ...
    newspaperImageWrapper.innerHTML = `
      <img id="newspaperImage" src="./img/newsboy_win_${performance}.png" alt="Photo of Chrono Delivery Kid posing">
    ...
    for (let i = 0; i < numPapers; i++) {
      ...
      if (!newspapersInfo[i].city && thisPerformance == 1) {
        articleText = articleTexts[1][1];
      }
      ...
      if (!newspapersInfo[i].city) {
        articleText = articleText.replace("NEWSPAPER_CITY, NEWSPAPER_STATE", `the United States`);
      } else {
  ...
  if (document.getElementById("playButton") && document.getElementById("settingsButton")) {
    document.getElementById("playScroll").remove();
    document.getElementById("settingsScroll").remove();

    let loadingScroll = document.createElement("div");
    loadingScroll.id = "loadingScroll";
    loadingScroll.innerHTML = `
      <p style="margin:0;" id="loadingText">Loading...</p>
    `;

    document.getElementById("canvasWrapper").appendChild(loadingScroll);
    resizeText();
  }
  ...
  if (!colonialBG) {
    ...
    assetPromises.push(loadImage(`./img/newsboy_hello.png`));
  }
  ...
  if (documents.length != numPapers) {
    ...
    gearSound.pause();
    home();
    alert("ERROR: Files too large to send to client");
    return;
  }
    ...
    guessWrapper.innerHTML = `
        <div id="guessBox">
          <div id="guessContent">
            ...
            <div id="guessButtons">
              <canvas id="boyAnimation" width="1920" height="1080" role="img" aria-label="Delivery kid animation"></canvas>
...
function resizeText() {
  ...
  if (document.getElementById("loadingText")) {
    document.getElementById("loadingText").style.fontSize = `${scalar / 4}px`;
  }
}
